...
// Adaptation : le nom de chaque crypto vient directement du backend
interface MarketItem {
  ...
  marketCap?: number;
  volume24h?: number;
  type: 'CRYPTO' | 'STOCK'; // Respecte le type reçu du backend
}
  ...
  const fetchMarketData = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erreur API Vercel: ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
      if (!Array.isArray(data) || data.length === 0) {
        toast.error('❌ Erreur : aucune donnée de marché reçue.');
      } else {
        // Filtre pour n'afficher que les cryptos et actions ayant un prix valide
        setMarketData(data.filter(item => typeof item.price === 'number' && !isNaN(item.price)));
        toast.success('Données du marché mises à jour !');
      }
    } catch (error: any) {
      console.error('Erreur lors de la récupération des données du marché:', error);
      toast.error(`❌ Récupération impossible : ${error.message}.`);
    } finally {
      setLoading(false);
    }
  }, []);
